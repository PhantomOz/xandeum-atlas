...
export function SeedControls({ discoveryList, respondingSeed, onConnect, onReset, isRefreshing }: SeedControlsProps) {
  ...
  return (
    <div className="w-full max-w-sm rounded-2xl border border-white/5 bg-slate-950/40 p-4 shadow-inner shadow-black/30">
      <p className="text-xs uppercase tracking-[0.3em] text-slate-500">Discovery list</p>
      <div className="mt-3 flex gap-2">
        <div className="flex w-full items-center gap-2 rounded-2xl border border-white/5 bg-slate-900/40 px-3 py-2">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-2xl border border-white/10 px-4 py-2 text-sm text-white hover:border-white/40 disabled:opacity-60"
        >
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-slate-400">
        <span>Responding seed: {respondingSeed}</span>
